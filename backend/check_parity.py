...
ROUTINE_TYPES_WITHOUT_RELEASE_GATES = {
    "field", "fields", "column", "columns", "filter", "filters",
    "label", "labels", "row", "control", "controls", "navigation",
}
...
def parse_registry_tables(text: str) -> tuple[list[dict[str, str]], list[str]]:
    ...
    expected_header = [
        "Slot", "Type", "Release gate", "Entitlement", "Org config",
        "Permission", "User hide", "Status", "Notes",
    ]
    i = 0
    while i < len(lines):
        line = lines[i].strip()
...
def route_to_page_file(route: str) -> Path:
    return FRONTEND_APP_ROOT / route.lstrip("/") / "page.tsx"
...
def validate_registry() -> tuple[list[str], dict[str, int]]:
    ...
    for route in routes:
        ...
        if not page_file.exists():
            errors.append(
                f"Registry route has no page source: {route} -> {page_file.relative_to(PROJECT_ROOT)}"
            )
    ...
    for row in rows:
        ...
        if gate not in {"—", "-", ""}:
            if not gate.startswith("release."):
                errors.append(
                    f"Registry line {line_no} ({slot}) release gate must start with 'release.': {gate}"
                )
        ...
        if org_config not in {"yes", "no", "—", "-", ""}:
            errors.append(
                f"Registry line {line_no} ({slot}) invalid Org config value: {row['Org config']}"
            )
        if user_hide not in {"yes", "no", "—", "-", ""}:
            errors.append(
                f"Registry line {line_no} ({slot}) invalid User hide value: {row['User hide']}"
            )
