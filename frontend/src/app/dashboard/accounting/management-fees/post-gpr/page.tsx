...
import {
  type ManagementFeeGPRCandidateList,
  listManagementFeeGPRCandidates,
  postManagementFeeGPR,
...
export default function ManagementFeePostGPRPage() {
  const { prefs } = useDisplay();
  const [month, setMonth] = useState(currentMonth);
  const [data, setData] = useState<ManagementFeeGPRCandidateList | null>(null);
  ...
  async function load() {
    ...
    setError("");
    try {
      const result = await listManagementFeeGPRCandidates(`${month}-01`);
      setData(result);
      setSelected(
        new Set(
          result.items
  ...
  async function submit() {
    ...
    setMessage("");
    try {
      const result = await postManagementFeeGPR(
        `${month}-01`,
        Array.from(selected)
      );
      setMessage(
        `Posted ${result.posted} GPR journal ${result.posted === 1 ? "entry" : "entries"}.`
      );
      await load();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Post Gross Potential Rent</h1>
        <p className="text-slate-500 mt-1">
          Uses the shared GPR accounting workflow. A unit/month already posted
          from Journal Entries or Management Fees cannot be posted again.
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-4 py-3 text-left">
                <input
                  type="checkbox"
                  aria-label="Select all unposted management fee GPR rows"
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">Loading GPR rows…</td></tr>
            )}
            {!loading && (data?.items.length ?? 0) === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">No active units with market rent were found.</td></tr>
            )}
            {!loading && data?.items.map((row) => (
              <tr key={row.unit_id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-3">
                  {row.already_posted ? (
                    <span className="text-xs text-green-700 bg-green-50 px-2 py-1 rounded-full">Posted</span>
                  ) : row.lease_id ? (
          ...
          {selectedRows.length > 0 && (
            <tfoot className="bg-slate-50 border-t border-slate-200 font-medium">
              <tr>
                <td colSpan={3} className="px-4 py-3 text-right">Selected totals</td>
